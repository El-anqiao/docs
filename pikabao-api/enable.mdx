...
<RequestExample>
```bash cURL
curl --location --request PUT 'http://kf.pikabao.cc/bank_card/enable?appid=user123&cardId=111222333&timestamp=1640995200000&sign=ABC123...'
```
...
const response = await fetch(
  `http://kf.pikabao.cc/bank_card/enable?${new URLSearchParams({...params, sign})}`,
  { method: "PUT" }
);
