...
<RequestExample>
```bash cURL
curl --location 'http://kf.pikabao.cc/agent/v1/cards/transactions?appid=user123&page=1&pageSize=20&timestamp=1640995200000&sign=ABC123...'
```
...
const response = await fetch(
  `http://kf.pikabao.cc/agent/v1/cards/transactions?${new URLSearchParams({...params, sign})}`
);
...
response = requests.get(
    "http://kf.pikabao.cc/agent/v1/cards/transactions",
    params=params
)
